...
Usage:
    python3 get_sql_connector_parameters.py                          # all connectors on demo
    python3 get_sql_connector_parameters.py --runtime demo --connector "SQL"
    python3 get_sql_connector_parameters.py --deployment test --runtime demo --all
    python3 get_sql_connector_parameters.py --profile ie_demo99_demo --connector "Kafka"
...
def select_process_groups(root_pg_id, connector_filter, recurse):
    """Select process groups to export.

    - recurse=False (default): immediate children of root (top-level connectors).
    - recurse=True: every process group at any depth.
    - connector_filter: optional case-insensitive substring on the group name.
    """
    if recurse:
...
def main():
    ...
    parser.add_argument(
        ...
        dest="connector",
        default=None,
        help="Case-insensitive substring to match the connector's process group name. "
        "Omit to export every connector on the runtime.",
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Recurse into every nested process group (not just top-level connectors).",
    )
    parser.add_argument(
        "--out-dir",
        default=".",
    ...
    try:
        ...
        matches = select_process_groups(root_pg_id, args.connector, args.all)
    ...
    if not matches:
        scope = f"matching '{args.connector}'" if args.connector else "found"
        print(
            f"No connector process group {scope} on profile '{args.profile}'.",
            file=sys.stderr,
        )
        sys.exit(2)
