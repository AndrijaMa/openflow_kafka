...
def resolve_profile(deployment, runtime):
    """Resolve a nipyapi profile from a deployment and/or runtime name via the cache."""
...
def main():
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Emit the result as JSON instead of formatted text.",
    )
    ...
    # Resolve which nipyapi profile to use.
    ...
    if args.profile:
        ...
        profile = resolve_profile(args.deployment, args.runtime)
    else:
