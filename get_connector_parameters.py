#!/usr/bin/env python3
"""
Retrieve the parameters of any connector running on an Openflow runtime.

Uses nipyapi to connect to the runtime (default: the `demo` runtime via the
`ie_demo99_demo` profile), locate connector process group(s), and print every
parameter in each bound parameter context. Exports one JSON file per connector.
